...
function submit_quiz_response()
{
	if(xhr_submit_quiz.readyState == 4)
	{
		...
		{
			var marks_index = xhr_submit_quiz.responseText.indexOf("MARKS");
			if( marks_index == 0 )
			{
				var score = xhr_submit_quiz.responseText.substring(5, xhr_submit_quiz.responseText.length);
				document.getElementById("course_code").value = course;
				document.getElementById("score").value = score;
				document.getElementById("quiz_name").value = quiz_name;
				document.getElementById("form_submit").click();
				//window.location = "./studentQuizSubmit.php?course=" + course + "&score=" + score;
			}
			else
			{
