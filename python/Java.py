...
class Java(Program):
	...
	def execute(self):
		...
		for test in test_data:
			try:
				cmd = "java " + m_file + " 1>student_output.txt 2>runtime_error.txt";
	...
	def staticAnalysis(self):
		...
		try:
			...
			count = 1;
			for f in self.files:
				temp_file = "analysis_error" + str(count) + ".txt";
				cmd = "../../../../../pmd-bin-5.2.3/bin/run.sh pmd -d " + f + " ";
				cmd = cmd + " -f text -R ../../../../../pmd-bin-5.2.3/MyRules.xml  -version 1.7 -language java 1>" + temp_file;
				cmd = cmd.strip();
				output = check_output(cmd, shell=True);
				count = count + 1;
		except Exception as e:
		...
		line = fd.read();
		while line:
			line = re.sub(r'/var/www/html/courseconnect/studentData/.*?/.*?/.*?/.*?/(.*?\.java)', r'\1', line.rstrip())
			error = error + line;
			line = fd.read();
		j = 1;
		while j < count:
			file_str = "analysis_error" + str(j) + ".txt";
			fd = open(file_str, "r");
			line = fd.read();
			while line:
				line = re.sub(r'/var/www/html/courseconnect/studentData/.*?/.*?/.*?/.*?/(.*?\.java)', r'\1', line.rstrip())
