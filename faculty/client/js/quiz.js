function show_new_quiz_form()
{
	$("#new_quiz_form").slideToggle("slow");
}

function show_quiz_questions_form()
{
	$("#new_quiz_questions_form").slideToggle("slow");
}
...
function generate_quiz(course_code)
{
	...
	{
		error_msg.innerHTML = "Select due date for quiz.";
		return;
	}
	if( due_time == "" || due_time == null )
	{
		error_msg.innerHTML = "Select due time for quiz.";
	}
	else
	{
		...
		var encodedDuedate = encodeNameAndValue("due_date", due_date);
		var encodedDuetime = encodeNameAndValue("due_time", due_time);
		...
		xhr_generate_quiz.send(encodedquizname + "&" + encodedqstntype + "&" + encodedeasy + "&" + encodedmed + "&" + encodedhard + "&" + encodedcourse + "&" + encodedtime + "&" + encodedenabled + "&" + encodedDuedate + "&" + encodedDuetime, true);
	}
}
...
function disable_quiz(quiz_id)
{
	...
	xhr_disable_quiz.open("GET", "../server/disable_delete_quiz.php?" + quiz + "&" + disable, true);
...
function delete_quiz(quiz_id)
{
	...
	{
		delete_row = document.getElementById("quiz" + quiz_id);
		...
		xhr_delte_quiz.open("GET", "../server/disable_delete_quiz.php?" + quiz + "&" + delete_q, true);
